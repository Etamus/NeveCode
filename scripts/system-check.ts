...
async function checkLlamaCppServer(): Promise<CheckResult> {
  ...
  if (!baseUrl) {
    return pass('llama-server', 'Skipped (OPENAI_BASE_URL not set).')
  }

  if (!isProviderLocalUrl(baseUrl)) {
    return pass('llama-server', 'Skipped (provider URL is not local).')
  }

  // Detect Atomic Chat (port 1337) - skip llamacpp check for it
  if (baseUrl.includes('1337')) {
    return pass('llama-server', 'Skipped (Atomic Chat local provider detected).')
  }

  const modelsUrl = `${getLlamaCppChatBaseUrl()}/models`
  try {
    const response = await fetch(modelsUrl, { signal: AbortSignal.timeout(3000) })
    if (!response.ok) {
    ...
    const models = (data.data ?? []).filter(m => m.id).map(m => m.id!)
    if (models.length === 0) {
