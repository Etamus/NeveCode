...
import { getOllamaChatBaseUrl, getLlamaCppChatBaseUrl } from './providerDiscovery.js'
...
export const PROFILE_FILE_NAME = '.nevecode-profile.json'
...
export type ProviderProfile = 'openai' | 'ollama' | 'codex' | 'gemini' | 'atomic-chat' | 'nvidia-nim' | 'minimax' | 'mistral' | 'llamacpp'
...
export function buildLlamaCppProfileEnv(options: {
  baseUrl?: string | null
  model?: string | null
}): ProfileEnv {
  return {
...
export async function buildLaunchEnv(options: {
  ...
  getLlamaCppChatBaseUrl?: (baseUrl?: string) => string
  ...
  if (options.profile === 'llamacpp') {
    const getLlamaCppBaseUrl =
      options.getLlamaCppChatBaseUrl ?? getLlamaCppChatBaseUrl
    env.OPENAI_BASE_URL = persistedEnv.OPENAI_BASE_URL || getLlamaCppBaseUrl()
    env.OPENAI_MODEL = persistedEnv.OPENAI_MODEL || persistedOpenAIModel || 'local-model'
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  // The legacy single-profile file (~/.nevecode-profile.json) is a
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.NEVECODE_PROFILE_GOAL),
    processEnv,
...
export async function applySavedProfileToCurrentSession(options: {
  ...
  const nextEnv = await buildLaunchEnv({
    ...
    goal: normalizeRecommendationGoal(processEnv.NEVECODE_PROFILE_GOAL),
    processEnv: baseEnv,
    getOllamaChatBaseUrl,
