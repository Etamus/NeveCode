...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), 'nevecode-env-test-'))
...
test('getGlobalClaudeFile: new install returns .nevecode.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.nevecode.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .nevecode.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.nevecode.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.nevecode.json'))
})
