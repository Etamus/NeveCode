...
test('CLI identity prefixes describe NeveCode instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('NeveCode')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('NeveCode')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes NeveCode instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('NeveCode')
  expect(prompt[0]).not.toContain('Claude Code')
  expect(prompt[0]).not.toContain("Anthropic's official CLI for Claude")
})

test('built-in agent prompts describe NeveCode instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('NeveCode')
  ...
  expect(generalPrompt).toContain('NeveCode')
  ...
  expect(explorePrompt).toContain('NeveCode')
  ...
  expect(guidePrompt).toContain('NeveCode')
  expect(guidePrompt).toContain('You are the NeveCode guide agent.')
  expect(guidePrompt).toContain('**NeveCode** (the CLI tool)')
