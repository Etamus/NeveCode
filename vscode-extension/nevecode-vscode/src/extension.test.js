...
function createStatus(overrides = {}) {
  return {
    ...
    shimEnabled: false,
    workspaceFolder: '/workspace/nevecode/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/nevecode/very/long/path/example-project',
    launchCwdLabel: '/workspace/nevecode/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/nevecode/very/long/path/example-project/.nevecode-profile.json',
    workspaceProfilePath: '/workspace/nevecode/very/long/path/example-project/.nevecode-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the NeveCode wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/nevecode\/very\/long\/path\/example-project"[^>]*>\/workspace\/nevecode\/very\/long\/path\/example-project<\//,
  );
});
...
test('NeveCodeControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { NeveCodeControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/nevecode/src/panels/control-center.js',
      workspacePath: '/workspace/nevecode',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/nevecode/src/panels',
      projectAwareCwdLabel: '/workspace/nevecode/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/nevecode',
      workspaceRootCwdLabel: '/workspace/nevecode',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/nevecode',
      activeFilePath: '/workspace/nevecode/src/panels/control-center.js',
      workspacePath: '/workspace/nevecode',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/nevecode',
      projectAwareCwdLabel: '/workspace/nevecode',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/nevecode/src/panels',
      launchCwdLabel: '/workspace/nevecode/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/nevecode\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/nevecode/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/nevecode/);
  assert.match(html, /Same workspace-root target as Launch NeveCode because the relative command resolves from the workspace root · \/workspace\/nevecode/);
});
