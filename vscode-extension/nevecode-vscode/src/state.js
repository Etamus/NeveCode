...
function getOpenAICompatibleLabel(baseUrl, model) {
  ...
  if (isLocalBaseUrl(normalizedBaseUrl)) {
    return 'Local compatível com OpenAI';
  }

  return 'Compatível com OpenAI';
}
...
function describeOpenAICompatible(env, source) {
  ...
  return buildProviderState(label, model || baseUrl || 'Runtime compatível com OpenAI', source);
}

function describeSavedProfile(profile) {
  switch (profile.profile) {
    case 'ollama':
      return buildProviderState('Ollama', getDetail(profile.env, 'perfil salvo'), 'profile');
    case 'gemini':
      return buildProviderState('Gemini', getDetail(profile.env, 'perfil salvo'), 'profile');
    case 'mistral':
      return buildProviderState('Mistral', getDetail(profile.env, 'perfil salvo'), 'profile')
    case 'codex':
      return buildProviderState('Codex', getDetail(profile.env, 'perfil salvo'), 'profile');
    case 'atomic-chat':
      return buildProviderState('Atomic Chat', getDetail(profile.env, 'perfil salvo'), 'profile');
    case 'openai':
    default:
...
function describeProviderState({ shimEnabled, env, profile }) {
  ...
  if (isEnvTruthy(env.CLAUDE_CODE_USE_GEMINI)) {
    return buildProviderState('Gemini', getDetail(env, 'via variáveis de ambiente'), 'env');
  }

  if (isEnvTruthy(env.CLAUDE_CODE_USE_MISTRAL)) {
    return buildProviderState('Mistral', getDetail(env, 'via variáveis de ambiente'), 'env');
  }

  if (isEnvTruthy(env.CLAUDE_CODE_USE_GITHUB)) {
    return buildProviderState('GitHub Models', getDetail(env, 'via variáveis de ambiente'), 'env');
  }

  if (isEnvTruthy(env.CLAUDE_CODE_USE_BEDROCK)) {
    return buildProviderState('Bedrock', 'via variáveis de ambiente', 'env');
  }

  if (isEnvTruthy(env.CLAUDE_CODE_USE_VERTEX)) {
    return buildProviderState('Vertex AI', 'via variáveis de ambiente', 'env');
  }

  if (isEnvTruthy(env.CLAUDE_CODE_USE_FOUNDRY)) {
    return buildProviderState('Foundry', 'via variáveis de ambiente', 'env');
  }
  ...
  if (shimEnabled) {
    return buildProviderState(
      'Compatível com OpenAI (provedor desconhecido)',
      'shim de início ativado',
      'shim',
    );
  }
  ...
  return buildProviderState(
    'Desconhecido',
    'nenhum perfil salvo ou variável de ambiente detectada',
    'unknown',
  );
}
